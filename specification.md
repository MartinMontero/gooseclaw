...
| O2 | **MCP-vault contract re-validation** against the shipped stateless MCP spec 2026-07-28 (and MCP-over-ACP assumptions) **before Phase 2** | **ANSWERED 2026-09-07** (Choice-B probe, `REPORT.md` — REPORTED): dual-era constraint folded into §3.2 as PROPOSED pending ratification. **Residual OPEN:** re-check each goose bump; sidecar pin 1.43.0 ≠ probed 1.48.0; HTTP/WS transports + mid-session `/mode` adversarial test unverified |
